...
const Womens = () => {
  ...
  const[loading,setloading]=useState(false)
    const getdata=(order,size,colors,filter)=>{
      let url;
      if(order){
        url=`http://localhost:8080/Womens?&_sort=price&_order=${order}`
      }else if(size){
        url=`http://localhost:8080/Womens?size=${size}&_sort=price&_order=${order}`
      }else if(colors){
        url=`http://localhost:8080/Womens?color=${colors}&_sort=price&_order=${order}`
      }
      else if(filter){
        url=`http://localhost:8080/Womens?material=${filter}&_sort=price&_order=${order}`
      }
  ...
  return (
    ...
    <Box>
      ...
      <Box w="100%" h="150px" mt="50px" >
        <Heading fontWeight={800} color="gray.600"> WOMEN’S OUTERWEAR</Heading>
